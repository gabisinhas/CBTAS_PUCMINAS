...
from flask import render_template, request, redirect, jsonify, session, url_for, Blueprint
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            if route.rule == "/<module_name>":
                ...
                if kwargs['module_name'] in ['check', 'review']:
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if 'email' in session:
                ...
                # --------------------------------------------------------------------
                # Check employee type because of NewCo database environment separation
                # --------------------------------------------------------------------
                # if env_type != "local":
                #     person_data = personDataByEmail(session['email'])
                #     if person_data['employeetype'].upper() == 'H':  # H for NewCo
                #         session['employeetype'] = 'NEWCO'
                #     else:
                #         session['employeetype'] = 'IBM'
...
@security.route("/security/login", methods=['GET', 'POST'])
def login():
    try:
        session.clear()
        ...
        if env_type == "local":
            ...
            session["serial_number"] = personDataByEmail(email="tfc@br.ibm.com")["serialnumber"]
            session['employeetype'] = 'IBM'
            session["roles"] = get_user_roles("tfc@br.ibm.com")
            return Redirect("/")
        ...
        logging.warn("A exception occurs during authentication: " + str(e))
        resp = jsonify({'status': 'not able to authenticate due an internal issue'})
        resp.status_code = 500
        return resp


@security.route("/security/logout")
def logout():
    if 'email' in session:
        session.clear()
        print(session)
...
@security.route('/oidc', methods=['GET', 'POST'])
def oidc_response():
    ...
    if aresp["state"] == session["state"] and mail:
        ...
        # --------------------------------------------------------------------
        person_data = personDataByEmail(session['email'])
        session["serial_number"] = person_data["serialnumber"]
        if person_data['employeetype'].upper() == 'H' or person_data['employeetype'].upper() == 'C':  # H and C for NewCo
            session['employeetype'] = 'NEWCO'
            logging.info("NEWCO user logged in")
        else:
            session['employeetype'] = 'IBM'
            logging.info("IBM user logged in")

        session["roles"] = get_user_roles(session["email"])
        logging.info("User roles loaded")

        # If user has provided a specific url
        if aresp["state"] in session:
            logging.info("User redirected to the requested page")
            return Redirect(session[aresp["state"]])
        else:
            logging.info("User redirected to the main page")
            return Redirect("/")
    else:
        logging.info("User redirected to the error page")
        return Redirect("/error")